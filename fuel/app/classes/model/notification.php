...
class Model_Notification extends \Orm\Model
{
    ...
    const POST_SENT = 11;
    const GETAWAY_RSVP_SENT = 12;
    const DATE_IDEA_SENT = 13;
    const PHOTO_UPLOADED = 14;
    const EVENT_INVITE_SENT = 15;
    const FRIEND_REQUEST_SENT = 16;
    ...
    const POST_NOTIFICATIONS = 1;
    const GETAWAY_NOTIFICATIONS = 1;
    const DATE_IDEA_NOTIFICATIONS = 1;
    const PHOTO_UPLOAD_NOTIFICATIONS = 1;
    ...
    public static function save_notifications($object_type, $object_id, $notified_to, $notified_by)
    {
        ...
        switch($object_type){
            ...
            case(self::EVENT_RSVP_SENT):
                ...
                break;
            case(self::EVENT_INVITE_SENT):
                $notification->category = self::EVENT_NOTIFICATIONS;
                break;
            case(self::FRIEND_REQUEST_SENT):
            ...
            case(self::REFERRED_A_FRIEND):
                $notification->category = self::FRIEND_NOTIFICATIONS;
                break;
            case(self::GETAWAY_RSVP_SENT):
                $notification->category = self::GETAWAY_NOTIFICATIONS;
                break;
    ...
    public static function get_public_notifications($notified_to)
    {
        $notifications = \Fuel\Core\DB::query("SELECT * FROM notifications
                            WHERE actor_id= $notified_to
                            AND (object_type_id = 11
                            OR object_type_id = 14)
    ...
    public static function count_invitations($id)
    {
        $notifications = \Fuel\Core\DB::query("SELECT * FROM notifications
                            WHERE subject_id=$id
                            AND (object_type_id = 12
                            OR object_type_id = 3)
