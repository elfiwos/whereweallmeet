...
class Quicksearch extends \Model {
    ...
    public static function get_random_matches($profile_id)
	{
		$count = 20;
        $obj_profile = \Model_Profile::find($profile_id);
		$gender_id = $obj_profile->gender_id;
		$ages_from = $obj_profile->ages_from;
		$ages_to = $obj_profile->ages_to;
		$zip_code = $obj_profile->zip;
        $year_from = (date("Y") - $ages_from);$month_from = date("m");$day_from = date("d");
	    $actual_ages_from = strtotime(date('Y-m-d', mktime(0, 0, 0, $month_from, $day_from, $year_from)));
	    $year_to = (date("Y") - $ages_to);$month_to = date("m");$day_to = date("d");
	    $actual_ages_to = strtotime(date('Y-m-d', mktime(0, 0, 0, $month_to, $day_to, $year_to)));

		$matched_members=array();

        $members_events = "(select distinct member_id from memberevents inner join events on memberevents.event_id = events.id where member_id <> $profile_id and type in (select distinct type from memberevents inner join events on memberevents.event_id = events.id where member_id = $profile_id))";
        $matched_member = \Fuel\Core\DB::query("Select profiles.id, city,first_name,last_name,picture,users.username,profiles.user_id,state,group_id, seeking_gender_id, ages_from, ages_to, relationship_status_id, want_kids, ethnicity_id, body_type_id
                    FROM profiles RIGHT JOIN users ON (users.id = profiles.user_id) WHERE
                    (profiles.id != '".$profile_id ."' AND is_activated = 1 AND Left(zip,2) = '". substr($zip_code,0,2) ."' AND birth_date BETWEEN ".$actual_ages_to." AND ".$actual_ages_from ." AND `gender_id` != '".$gender_id."')
                    OR (profiles.id != '".$profile_id ."' AND is_activated = 1 AND Left(zip,2) = '". substr($zip_code,0,2) ."' AND `gender_id` != '".$gender_id."')
                    OR (profiles.id != '".$profile_id ."' AND is_activated = 1 AND profiles.id in $members_events)
                    ORDER BY " . DB::expr('RAND()'). "LIMIT ".$count)->execute();
        foreach($matched_member as $value) {
            array_push($matched_members, $value);
        }
		return $matched_members;
	}

   public static function get_seeking_gender($username, $password)
	{
		$id = Quicksearch::get_id($username, $password);
		$result = DB::select('seeking_gender_id')
		->from('profiles')
		->where('user_id', $id)
		->execute();
		return $result[0]['seeking_gender_id'];
	}
	...
	public static function get_dating_agent_result($profile_id)
	{
		$count = 20;
	...
	public static function get_zip_code($username, $password) 
	{
		$id = Quicksearch::get_id($username, $password);
		$result = DB::select('zip')
		->from('profiles')
		->where('user_id', $id)
		->execute();
		return $result[0]['zip'];
    ...
    public static function browse_members($search_param)
  {
        ...
        if(!empty($search_param["key_words"])) {
          $result = $result->and_where_open()
              ->where('username','like','%'.$search_param["key_words"].'%')
              ->or_where('email','like','%'.$search_param["key_words"].'%')
              ->and_where_close();
        }
        ...
        $result = $result->order_by(DB::expr('RAND()'));
