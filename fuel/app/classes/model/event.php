...
class Model_Event extends \Orm\Model
{
    ...
    public static  function is_active_event($event_id) {
        $result = \Fuel\Core\DB::query("SELECT * FROM events WHERE id=". $event_id ." AND event_end_date >= CURDATE()")->execute();
        if(count($result) > 0) {
            return true;
        } else {
            return false;
    ...
    public static function find_by_slug($event_slug)
    {
        $result = \Fuel\Core\DB::query("SELECT id, title, slug, long_description, short_description, url, venue,
        DATE_FORMAT(start_date,'%W, %M %d, %Y ') AS start_date, TIME_FORMAT(start_time, '%h:%i %p') AS start_time, photo,
        state, city, TIME_FORMAT(end_time, '%h:%i %p') AS end_time, youtube_video, created_at, updated_at
