...
class Model_Friendship extends \Orm\Model
{
    ...
    public static  function search_friends($profile_id, $username) {
        $friend_ids = array();
        $user_ids = array();
        ...
        $users = DB::query("SELECT * FROM users WHERE username LIKE '%" . $username . "%'")
            ->as_object('Model_Users')->execute();
        foreach ($users as $user) {
            array_push($user_ids, $user->id);
        }

        return count($friend_ids) > 0 ? Model_Profile::query()
            ->where("id", "IN", $friend_ids)->where("user_id", "IN", $user_ids)
            ->get() : array();
    }
    ...
    public static function count_pending_friends($profile_id) {
        $friend_ids = array();
    ...
    public static function get_sent_friend_requests($sender_id) {
        $friend_ids = array();

        $friendships = DB::query("SELECT * FROM friendships WHERE sender_id = $sender_id ")
            ->as_object('Model_Friendship')->execute();
