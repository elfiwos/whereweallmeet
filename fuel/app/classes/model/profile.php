...
class Model_Profile extends \Orm\Model
{
    ...
    protected static $_properties = array(
        ...
        'send_me_listing_info',
        'send_me_date_invitations',
        'send_me_announcement_info',
    ...
    public static function get_picture($picture, $user_id, $type)
    {
        ...
        if (isset($picture) && $picture != "" && file_exists("uploads/" . Model_Profile::clean_name($user['username']) . "/" . $picture)) {
            if($type == "original") {
            ...
            elseif($type == "referral") {
                ...
                if(file_exists($file_name)) {
                    ...
                    $original_file = "uploads/" . Model_Profile::clean_name($user['username']) . "/" . $picture;
                    $save_as = "uploads/" . Model_Profile::clean_name($user['username']) . "/" . $type . "_" . $picture;
                    Image::load($original_file)->crop_resize(88, 87)->save($save_as);
                    return Uri::create("uploads/" . Model_Profile::clean_name($user['username']) . "/" . $type . "_" . $picture);
                }
            }
            elseif($type == "referred_friend") {
                $file_name = "uploads/" . Model_Profile::clean_name($user['username']) . "/" . $type . "_" . $picture;
                if(file_exists($file_name)) {
                    return Uri::create("uploads/" . Model_Profile::clean_name($user['username']) . "/" . $type . "_" . $picture);
                }
                    ...
                    $original_file = "uploads/" . Model_Profile::clean_name($user['username']) . "/" . $picture;
                    $save_as = "uploads/" . Model_Profile::clean_name($user['username']) . "/" . $type . "_" . $picture;
                    Image::load($original_file)->crop_resize(400, 400)->save($save_as);
                    return Uri::create("uploads/" . Model_Profile::clean_name($user['username']) . "/" . $type . "_" . $picture);
                }
            }
            else {
    ...
    public static function online_dating_agents_count() {
        $result = Model_Profile::find('all', array(
            'where' => array('is_logged_in' => 1, 'member_type_id' => Model_Membershiptype::DATING_AGENT_MEMBER)
        ));
        return count($result);
    }
    ...
    public static function count_referals($profile_id){
        $query = DB::select('*')->from('refer_friends');
        $query->where('refer_to', $profile_id);
