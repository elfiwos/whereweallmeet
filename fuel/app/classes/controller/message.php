...
class Controller_Message extends Controller_Base {
    ...
    public function action_delete_message() {
        ...
        if (Input::method() == 'POST' or Input::is_ajax()) {
            ...
            $message_section = Input::post("message_section");

            $message =Model_Message::find($message_id);
            if ($message) {
                if($message_section == "inbox"){
                    $message->trash_inbox_id = $this->current_profile->id;
                    $message->save();
                } else if($message_section == "inbox-detail"){
                    $message->trash_inbox_id = $this->current_profile->id;
                    $message->save();
                } else if ($message_section == "sent") {
                    $message->trash_sent_id = $this->current_profile->id;
                    $message->save();
                } else if ($message_section == "trash-inbox") {
                    $message->is_deleted_reciever_forever = 1;
                    $message->save();
                } else if ($message_section == "trash-sent") {
                    $message->is_deleted_sender_forever = 1;
                    $message->save();
                }
    ...
    public function action_message_detail($message_id = null){
        if (Input::method() == 'POST') {
            ...
            if ($val->run()) {
                $message = Model_Message::forge(array(
                    ...
                    'message_status' => Model_Message::STATUS_UNREAD,
                    'is_deleted_sender' => 0,
                    'is_deleted_receiver' => 0,
                    'parent_message_id' => Input::post('parent'),
                    'archive_inbox' => 0,
                    'archive_sent' => 0,
                ...
                if ($message and $message->save()) {
                    if (Model_Notification::save_notifications(Model_Notification::MESSAGE_SENT, $message->id, $message->to_member_id, $message->from_member_id))
                    {
                        ...
                        Email::forge()->to(Model_Profile::get_email($notification_profile->user_id))->from("admin@whereweallmeet.com")->subject("WhereWeAllMeet Notification")
                            ->html_body(View::forge('email/notification', array("notification" => Model_Profile::get_username($notification_profile->user_id) ." has sent you a message.")))->priority(\Email\Email::P_HIGH)->send();
        ...
        $view = View::forge('messages/message_detail');

        $objMessage = Model_Message::find($message_id);
        if($objMessage) {
            $objMessage->message_status = Model_Message::STATUS_READ;
            $objMessage->save();
        }
        $view->message = $objMessage;

        $re_message = Model_Message::query()->where("parent_message_id", $message_id)->where("trash_sent_id", 0)->order_by('created_at', 'desc')->get();
        $view->re_message = $re_message;
    ...
    public function action_dashboard_compose(){
            ...
            else if($username == 'All Clients')
            {
                ...
                if ($message and $message->save()) {
                    if (
                        ...
                        //print_r($this->current_profile->first_name);
                       // print_r($this->current_profile->last_name);
                      //  die;
                        Email::forge()->to($notification_user->email)->from("admin@whereweallmeet.com")->subject("WhereWeAllMeet Notification")
                            ->html_body(View::forge('email/notification', array("notification" => $this->current_profile->first_name . ' ' . $this->current_profile->last_name ." has sent you a message.")))->priority(\Email\Email::P_HIGH)->send();
    ...
    public function action_create_ajax() {
        ...
        if ($val->run()) {
            ...
            $post = Input::post();
            $post["date_sent"] = date('Y-m-d H:i:s', Date::forge()->get_timestamp());
