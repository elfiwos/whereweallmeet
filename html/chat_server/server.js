...
io.sockets.on('connection', function(socket) {
   ...
   socket.on('send_message', function(data) {
      ...
      var chat_room_id = data.chat_room_id;
      var r_users = get_room_users(data.chat_room_id);
      var message_reciever;

     
      r_users.forEach(function(theuser){
        if(theuser == socket.nickname ){
          console.log("this is the sender");
        }else{
          message_reciever = theuser;
        }
        
      });

      io.sockets.in(chat_room_id).emit('chat', {chat_room_id: chat_room_id, message: data.message, m_reciever: message_reciever, nickname: socket.nickname});
